...
#define SafeSwapFloat( pOut, pIn )	(*((uint32_t*)pOut) = DWordSwapC( *((uint32_t*)pIn) ))
...
{
public:
	CBitRead(const void* pData, size_t nBytes, long long nBits = -1);
	CBitRead(const char* pDebugName, const void* pData, size_t nBytes, long long nBits = -1);
	CBitRead(void) : CBitBuffer()
	{
	}

	void StartReading(const void* pData, size_t nBytes, long long iStartBit = 0, long long nBits = -1);

	FORCEINLINE long long Tell(void) const;
	bool Seek(long long nPosition);

	FORCEINLINE bool SeekRelative(long long nOffset)
	{
	...
	FORCEINLINE long long GetNumBitsRead(void)     const { return Tell(); };
	FORCEINLINE long long GetNumBytesRead(void)    const { return ((GetNumBitsRead() + 7) >> 3); }

	FORCEINLINE long long GetNumBitsLeft(void)     const { return m_nDataBits - GetNumBitsRead(); }
	FORCEINLINE long long GetNumBytesLeft(void)    const { return GetNumBitsLeft() >> 3; }
	...
	FORCEINLINE int ReadLong(void)  { return ReadUBitLong(sizeof(int32_t) << 3); }
	FORCEINLINE float ReadFloat(void)
	{
		uint32_t nUval = ReadUBitLong(sizeof(int32_t) << 3);
		return *((float*)&nUval);
	}

	// reads a signed 64-bit integer from the buffer
	int64_t           ReadLongLong(void);

	// reads a varint encoded integer
	uint32_t          ReadVarInt32();
	uint64_t          ReadVarInt64();
	int32_t           ReadSignedVarInt32() { return bitbuf::ZigZagDecode32(ReadVarInt32()); }
	int64_t           ReadSignedVarInt64() { return bitbuf::ZigZagDecode64(ReadVarInt64()); }
...
//-----------------------------------------------------------------------------
FORCEINLINE long long CBitRead::Tell(void) const
{
	...
	long long nCurOfs = int64_t(((INT_PTR(m_pDataIn) - INT_PTR(m_pData)) / 4) - 1);
	nCurOfs *= 32;
	nCurOfs += (32 - m_nBitsAvail);
	long long nAdjust = 8 * (m_nDataBytes & 3);
