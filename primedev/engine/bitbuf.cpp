...
// bf_read
// ---------------------------------------------------------------------------------------- //
CBitRead::CBitRead(const void* pData, size_t nBytes, long long nBits /*= -1*/)
{
	StartReading(pData, nBytes, 0, nBits);
}
CBitRead::CBitRead(const char* pDebugName, const void* pData, size_t nBytes, long long nBits /*= -1*/)
{
...
//-----------------------------------------------------------------------------
void CBitRead::StartReading(const void* pData, size_t nBytes, long long iStartBit, long long nBits)
{
	// Make sure it's dword aligned and padded.
	assert((long long(pData) & 3) == 0);
	...
	{
		assert(nBits <= (long long)(nBytes * 8));
		m_nDataBits = nBits;
	}
...
bool CBitRead::Seek(long long nPosition)
{
	bool bSucc = true;
	...
	{
		long long nAdjPosition = nPosition - (nHead << 3);
...
{
	int64_t retval;
	uint32_t* pLongs = (uint32_t*)&retval;
...
uint32_t CBitRead::ReadVarInt32()
{
	uint32_t result = 0;
	int count = 0;
	uint32_t b;
...
//-----------------------------------------------------------------------------
uint64_t CBitRead::ReadVarInt64()
{
	uint64_t result = 0;
	int count = 0;
	uint64_t b;
...
//-----------------------------------------------------------------------------
bool CBitRead::ReadWString(_Out_z_cap_(maxLenInChars) wchar_t* pStr, int maxLenInChars, bool bLine, int* pOutNumChars)
{
...
//-----------------------------------------------------------------------------
void CBitWrite::WriteVarInt32(uint32_t data)
{
...
//-----------------------------------------------------------------------------
void CBitWrite::WriteVarInt64(uint64_t data)
{
	// Check if align and we have room, slow path if not
...
//-----------------------------------------------------------------------------
void CBitWrite::WriteSignedVarInt32(int32_t data)
{
	WriteVarInt32(bitbuf::ZigZagEncode32(data));
}

//-----------------------------------------------------------------------------
void CBitWrite::WriteSignedVarInt64(int64_t data)
{
	WriteVarInt64(bitbuf::ZigZagEncode64(data));
}

//-----------------------------------------------------------------------------
int CBitWrite::ByteSizeVarInt32(uint32_t data)
{
	int size = 1;
	while (data > 0x7F) {
		size++;
...
//-----------------------------------------------------------------------------
int CBitWrite::ByteSizeVarInt64(uint64_t data)
{
	int size = 1;
	while (data > 0x7F) {
		size++;
...
//-----------------------------------------------------------------------------
int CBitWrite::ByteSizeSignedVarInt32(int32_t data)
{
	return ByteSizeVarInt32(bitbuf::ZigZagEncode32(data));
}

//-----------------------------------------------------------------------------
int CBitWrite::ByteSizeSignedVarInt64(int64_t data)
{
...
//-----------------------------------------------------------------------------
void CBitWrite::WriteLongLong(int64_t val)
{
