...
{
public:
	CUtlCStringConversion(char nEscapeChar, const char* pDelimiter, int64_t nCount, ConversionArray_t* pArray);

	// Finds a conversion for the passed-in string, returns length
	virtual char FindConversion(const char* pString, int64_t* pLength);
...
{
public:
	CUtlNoEscConversion(char nEscapeChar, const char* pDelimiter, int64_t nCount, ConversionArray_t* pArray) :
		CUtlCharConversion(nEscapeChar, pDelimiter, nCount, pArray) {}

	// Finds a conversion for the passed-in string, returns length
	virtual char FindConversion(const char* pString, int64_t* pLength) { *pLength = 0; return 0; }
};
...
// Constructor
//-----------------------------------------------------------------------------
CUtlCStringConversion::CUtlCStringConversion(char nEscapeChar, const char* pDelimiter, int64_t nCount, ConversionArray_t* pArray) :
	CUtlCharConversion(nEscapeChar, pDelimiter, nCount, pArray)
{
...
// Finds a conversion for the passed-in string, returns length
char CUtlCStringConversion::FindConversion(const char* pString, int64_t* pLength)
{
...
// Constructor
//-----------------------------------------------------------------------------
CUtlCharConversion::CUtlCharConversion(char nEscapeChar, const char* pDelimiter, int64_t nCount, ConversionArray_t* pArray)
{
...
int64_t CUtlCharConversion::GetDelimiterLength() const
{
...
int64_t CUtlCharConversion::GetConversionLength(char c) const
{
	return m_pReplacements[(unsigned char)c].m_nLength;
}

int64_t CUtlCharConversion::MaxConversionLength() const
{
...
// Finds a conversion for the passed-in string, returns length
//-----------------------------------------------------------------------------
char CUtlCharConversion::FindConversion(const char* pString, int64_t* pLength)
{
	for (int64_t i = 0; i < m_nCount; ++i)
	{
...
// constructors
//-----------------------------------------------------------------------------
CUtlBuffer::CUtlBuffer(int64_t growSize, int64_t initSize, int nFlags) :
	m_Error(0)
{
	//MEM_ALLOC_CREDIT();
...
CUtlBuffer::CUtlBuffer(const void* pBuffer, int64_t nSize, int nFlags) :
...
//-----------------------------------------------------------------------------
void CUtlBuffer::SetExternalBuffer(void* pMemory, int64_t nSize, int64_t nInitialPut, int nFlags)
{
...
//-----------------------------------------------------------------------------
void CUtlBuffer::AssumeMemory(void* pMemory, int64_t nSize, int64_t nInitialPut, int nFlags)
{
...
//-----------------------------------------------------------------------------
void CUtlBuffer::EnsureCapacity(int64_t num)
{
	//MEM_ALLOC_CREDIT();
...
//-----------------------------------------------------------------------------
bool CUtlBuffer::Get(void* pMem, int64_t size)
{
...
//-----------------------------------------------------------------------------
int64_t CUtlBuffer::GetUpTo(void* pMem, int64_t nSize)
{
...
//-----------------------------------------------------------------------------
int64_t CUtlBuffer::PeekWhiteSpace(int64_t nOffset)
{
	if (!IsText() || !IsValid())
		return 0;
...
//-----------------------------------------------------------------------------
int64_t CUtlBuffer::PeekStringLength()
{
	if (!IsValid())
		return 0;
	...
	int64_t nStartingOffset = nOffset;

	do
	{
		int64_t nPeekAmount = 128;
		...
		if (!IsText())
		{
			for (int64_t i = 0; i < nPeekAmount; ++i)
			{
...
//-----------------------------------------------------------------------------
int64_t CUtlBuffer::PeekLineLength()
{
	if (!IsValid())
		return 0;
...
//-----------------------------------------------------------------------------
bool CUtlBuffer::PeekStringMatch(int64_t nOffset, const char* pString, int64_t nLen)
{
...
//-----------------------------------------------------------------------------
int64_t CUtlBuffer::PeekDelimitedStringLength(CUtlCharConversion* pConv, bool bActualSize)
{
	if (!IsText() || !pConv)
	...
	int64_t nActualStart = nOffset;
	nOffset += pConv->GetDelimiterLength();
	int64_t nLen = 1;	// Starts at 1 for the '\0' termination
	...
	do
	{
		...
		{
			int64_t nLength = pConv->MaxConversionLength();
...
//-----------------------------------------------------------------------------
void CUtlBuffer::GetString(char* pString, int64_t nMaxChars)
{
	if (!IsValid())
	{
		*pString = 0;
	...
	const int64_t nCharsToRead = std::min(nLen, nMaxChars) - 1;
...
//-----------------------------------------------------------------------------
void CUtlBuffer::GetLine(char* pLine, int64_t nMaxChars)
{
...
void CUtlBuffer::GetDelimitedString(CUtlCharConversion* pConv, char* pString, int64_t nMaxChars)
{
	if (!IsText() || !pConv)
	{
...
//-----------------------------------------------------------------------------
bool CUtlBuffer::CheckGet(int64_t nSize)
{
...
//-----------------------------------------------------------------------------
bool CUtlBuffer::CheckPeekGet(int64_t nOffset, int64_t nSize)
{
	if (m_Error & GET_OVERFLOW)
		return false;
...
//-----------------------------------------------------------------------------
bool CUtlBuffer::CheckArbitraryPeekGet(int64_t nOffset, int64_t& nIncrement)
{
	...
	int64_t nMaxGet = TellMaxPut() - TellGet();
...
//-----------------------------------------------------------------------------
const void* CUtlBuffer::PeekGet(int64_t nMaxSize, int64_t nOffset)
{
...
//-----------------------------------------------------------------------------
void CUtlBuffer::SeekGet(SeekType_t type, int64_t offset)
{
	switch (type)
	{
	case SEEK_HEAD:
...
int64_t CUtlBuffer::VaScanf(const char* pFmt, va_list list)
{
	if (m_Error || !IsText())
		return 0;
...
int64_t CUtlBuffer::Scanf(const char* pFmt, ...)
{
	va_list args;

	va_start(args, pFmt);
	int64_t count = VaScanf(pFmt, args);
	va_end(args);
...
bool CUtlBuffer::GetToken(const char* pToken)
{
	...
	// Look for the token
	int64_t nLen = V_strlen(pToken);

	// First time through on streaming, check what we already have loaded
	// if we have enough loaded to do the check
	int64_t nMaxSize = Size() - (TellGet() - m_nOffset);
	if (nMaxSize <= nLen)
	{
		nMaxSize = Size();
	}
	int64_t nSizeRemaining = TellMaxPut() - TellGet();

	int64_t nGet = TellGet();
	while (nSizeRemaining >= nLen)
	{
		bool bOverFlow = (nSizeRemaining > nMaxSize);
		int64_t nSizeToCheck = bOverFlow ? nMaxSize : nSizeRemaining;
		...
		bool bPotentialMismatch = (bOverFlow && ((int64_t)nOffset == Size() - nLen));
		...
		SeekGet(CUtlBuffer::SEEK_CURRENT, (int64_t)(nOffset + nLen));
		return true;
	}
...
//-----------------------------------------------------------------------------
bool CUtlBuffer::ParseToken(const char* pStartingDelim, const char* pEndingDelim, char* pString, int64_t nMaxLen)
{
	int64_t nCharsToCopy = 0;
	int64_t nCurrentGet = 0;
	...
	int64_t nStartGet = TellGet();
	int64_t nTokenStart = -1;
	char nCurrChar;
	EatWhiteSpace();
...
//-----------------------------------------------------------------------------
int64_t CUtlBuffer::ParseToken(characterset_t* pBreaks, char* pTokenBuf, int64_t nMaxLen, bool bParseComments)
{
	pTokenBuf[0] = 0;
...
// Serialization
//-----------------------------------------------------------------------------
void CUtlBuffer::Put(const void* pMem, int64_t size)
{
...
void CUtlBuffer::PutString(const char* pString)
{
	if (!IsText())
	{
		if (pString)
		{
			...
			int64_t nLen = V_strlen(pString) + 1;
	...
	{
		int64_t nTabCount = (m_Flags & AUTO_TABS_DISABLED) ? 0 : m_nTab;
		if (nTabCount > 0)
		{
...
{
	int64_t l = pConv->GetConversionLength(c);
	if (l == 0)
	{
		PutChar(c);
	}
...
{
	char temp[8192];
	//int64_t nLen = V_vsnprintf(temp, sizeof(temp), pFmt, list);
...
// Calls the overflow functions
//-----------------------------------------------------------------------------
bool CUtlBuffer::OnPutOverflow(int64_t nSize)
{
	return (this->*m_PutOverflowFunc)(nSize);
}

bool CUtlBuffer::OnGetOverflow(int64_t nSize)
{
...
//-----------------------------------------------------------------------------
bool CUtlBuffer::PutOverflow(int64_t nSize)
{
	//MEM_ALLOC_CREDIT();
...
bool CUtlBuffer::GetOverflow(int64_t nSize)
{
	return false;
}
...
// Checks if a put is ok
//-----------------------------------------------------------------------------
bool CUtlBuffer::CheckPut(int64_t nSize)
{
...
void CUtlBuffer::SeekPut(SeekType_t type, int64_t offset)
{
	int64_t nNextPut = m_Put;
	switch (type)
	{
	case SEEK_HEAD:
		nNextPut = offset;
...
//-----------------------------------------------------------------------------
void CUtlBuffer::AddNullTermination(int64_t nPut)
{
	if (nPut > m_nMaxPut)
	{
...
bool CUtlBuffer::ConvertCRLF(CUtlBuffer& outBuf)
{
	...
	while (nCurrGet < nInCount)
	{
		...
		if (bFromCRLF)
		{
			const char* pNext = V_strnistr(pCurr, "\r\n", (int64_t)nInCount - (int64_t)nCurrGet);
			if (!pNext)
			{
		...
		else
		{
			...
			if (!pNext)
			{
				outBuf.Put(pCurr, (int64_t)nInCount - (int64_t)nCurrGet);
				break;
			}

			intptr_t nBytes = (intptr_t)pNext - (intptr_t)pCurr;
			outBuf.Put(pCurr, (int64_t)nBytes);
			outBuf.PutChar('\r');
			outBuf.PutChar('\n');
			nCurrGet += nBytes + 1;
			if (nGet >= nCurrGet)
...
CUtlInplaceBuffer::CUtlInplaceBuffer(int64_t growSize /* = 0 */, int64_t initSize /* = 0 */, int nFlags /* = 0 */) :
	CUtlBuffer(growSize, initSize, nFlags)
{
}

bool CUtlInplaceBuffer::InplaceGetLinePtr(char** ppszInBufferPtr, int64_t* pnLineLength)
{
	int64_t nLineLen = PeekLineLength();
	if (nLineLen <= 1)
	{
		SeekGet(SEEK_TAIL, 0);
		return false;
