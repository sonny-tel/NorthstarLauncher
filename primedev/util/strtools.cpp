...
//-----------------------------------------------------------------------------
char* AllocString(const char* pStr, int64_t nMaxChars)
{
	const int64_t allocLen = (nMaxChars == -1)
		? strlen(pStr) + 1
		: std::min((int64_t)strlen(pStr), nMaxChars) + 1;
...
int	_V_stricmp_NegativeForUnequal(const char* s1, const char* s2)
{
	...
	uint8_t const* pS1 = (uint8_t const*)s1;
	uint8_t const* pS2 = (uint8_t const*)s2;
...
//			maxoutputbytes -
//-----------------------------------------------------------------------------
void V_hextobinary(char const* in, size_t numchars, uint8_t* out, size_t maxoutputbytes)
{
...
//			outsize -
//-----------------------------------------------------------------------------
void V_binarytohex(const uint8_t* in, size_t inputbytes, char* out, size_t outsize)
{
	char doublet[10];
	int i;
...
int64_t V_vsnprintfRet(char* pDest, size_t maxLen, const char* pFormat, va_list params, bool* pbTruncated)
{
	int64_t len = _vsnprintf(pDest, maxLen, pFormat, params);
	const bool bTruncated = (len < 0) || (len >= (int64_t)maxLen);
...
void V_SplitString2(const char* pString, const char** pSeparators, int64_t nSeparators, CUtlStringList& outStrings)
{
	outStrings.Purge();
	...
	while (true)
	{
		int64_t iFirstSeparator = -1;
		const char* pFirstSeparator = nullptr;

		for (int64_t i = 0; i < nSeparators; i++)
		{
		...
		if (pFirstSeparator)
		{
			...
			const int64_t separatorLen = strlen(pSeparators[iFirstSeparator]);

			if (pFirstSeparator > pCurPos)
			{
				const int64_t nLen = pFirstSeparator - pCurPos;
...
void V_AppendSlash(char* pStr, size_t strSize, char separator)
{
	...
	if (len > 0 && !PATHSEPARATOR(pStr[len - 1]))
	{
		if (len + 1 >= strSize)
		{
			spdlog::error("V_AppendSlash: ran out of space on {}.", pStr);
			exit(EXIT_FAILURE);
		}
...
#pragma warning(disable : 4267)
				...
				{
					spdlog::error("V_MakeAbsolutePath: _getcwd failed.");
					exit(EXIT_FAILURE);
				}
#pragma warning(pop)
			}
	...
	{
		spdlog::error("V_MakeAbsolutePath: tried to \"..\" past the root.");
		exit(EXIT_FAILURE);
	}
...
bool V_ExtractFilePath(const char* path, char* dest, size_t destSize)
{
	...
	const int64_t copysize = std::min(size_t(src - path), destSize - 1);
...
void V_FileBase(const char* in, char* out, size_t maxlen)
{
	...
	size_t maxcopy = std::min(len + 1, maxlen);
