...
public:
	...
	CUtlCharConversion(char nEscapeChar, const char* pDelimiter, int64_t nCount, ConversionArray_t* pArray);
	char GetEscapeChar() const;
	const char* GetDelimiter() const;
	int64_t GetDelimiterLength() const;

	const char* GetConversionString(char c) const;
	int64_t GetConversionLength(char c) const;
	int64_t MaxConversionLength() const;
...
protected:
	...
	int64_t m_nDelimiterLength;
	int64_t m_nCount;
	int64_t m_nMaxConversionLength;
	char m_pList[256];
...
template <> const char* GetFmtStr< short >(int nRadix, bool bPrint) { return "%hd"; }
template <> const char* GetFmtStr< ushort >(int nRadix, bool bPrint) {  return "%hu"; }
template <> const char* GetFmtStr< int >(int nRadix, bool bPrint) {  return "%d"; }
template <> const char* GetFmtStr< uint32_t >(int nRadix, bool bPrint) {  return nRadix == 16 ? "%x" : "%u"; }
template <> const char* GetFmtStr< int64_t >(int nRadix, bool bPrint) {  return "%lld"; }
template <> const char* GetFmtStr< float >(int nRadix, bool bPrint) {  return "%f"; }
template <> const char* GetFmtStr< double >(int nRadix, bool bPrint) { return bPrint ? "%.15lf" : "%lf"; } // force Printf to print DBL_DIG=15 digits of precision for doubles - defaults to FLT_DIG=6
#else
...
template <> static const char* GetFmtStr< int64_t >(int nRadix, bool bPrint) { Assert(nRadix == 10); return "%lld"; }
...
public:
	...
	typedef bool (CUtlBuffer::* UtlBufferOverflowFunc_t)(int64_t nSize);

	// Constructors for growable + external buffers for serialization/unserialization
	CUtlBuffer(int64_t growSize = 0, int64_t initSize = 0, int nFlags = FLAGS_NONE);
	CUtlBuffer(const void* pBuffer, int64_t size, int nFlags = FLAGS_NONE);
	// This one isn't actually defined so that we catch constructors that are trying to pass a bool in as the third param.
	CUtlBuffer(const void* pBuffer, int64_t size, bool crap) = delete;
	...
	void* AccessForDirectRead(int64_t nBytes);

	// Attaches the buffer to external memory....
	void			SetExternalBuffer(void* pMemory, int64_t nSize, int64_t nInitialPut, int nFlags = 0);
	bool			IsExternallyAllocated() const;
	void			AssumeMemory(void* pMemory, int64_t nSize, int64_t nInitialPut, int nFlags = 0);
	void* Detach();
	void* DetachMemory();

	// copies data from another buffer
	void			CopyBuffer(const CUtlBuffer& buffer);
	void			CopyBuffer(const void* pubData, int64_t cubData);
	...
	uint64_t			GetUnsignedint64_t();
	float			GetFloat();
	double			GetDouble();
	void* GetPtr();
	void			GetString(char* pString, int64_t nMaxChars);
	bool			Get(void* pMem, int64_t size);
	void			GetLine(char* pLine, int64_t nMaxChars);

	// Used for getting objects that have a byteswap datadesc defined
	template <typename T> void GetObjects(T* dest, int64_t count = 1);

	// This will get at least 1 byte and up to nSize bytes. 
	// It will return the number of bytes actually read.
	int64_t			GetUpTo(void* pMem, int64_t nSize);

	// This version of GetString converts \" to \\ and " to \, etc.
	// It also reads a " at the beginning and end of the string
	void			GetDelimitedString(CUtlCharConversion* pConv, char* pString, int64_t nMaxChars = 0);
	...
	int64_t			PeekDelimitedStringLength(CUtlCharConversion* pConv, bool bActualSize = true);

	// Just like scanf, but doesn't work in binary mode
	int64_t			Scanf(SCANF_FORMAT_STRING const char* pFmt, ...);
	int64_t			VaScanf(const char* pFmt, va_list list);
	...
	bool			ParseToken(const char* pStartingDelim, const char* pEndingDelim, char* pString, int64_t nMaxLen);
	...
	int64_t			ParseToken(characterset_t* pBreaks, char* pTokenBuf, int64_t nMaxLen, bool bParseComments = true);
	...
	void* PeekPut(int64_t offset = 0);
	const void* PeekGet(int64_t offset = 0) const;
	const void* PeekGet(int64_t nMaxSize, int64_t nOffset);

	// Where am I writing (put)/reading (get)?
	int64_t TellPut() const;
	int64_t TellGet() const;

	// What's the most I've ever written?
	int64_t TellMaxPut() const;
	...
	void SeekPut(SeekType_t type, int64_t offset);
	void SeekGet(SeekType_t type, int64_t offset);
...
protected:
	...
	bool OnPutOverflow(int64_t nSize);
	bool OnGetOverflow(int64_t nSize);

protected:
	// Checks if a get/put is ok
	bool CheckPut(int64_t size);
	bool CheckGet(int64_t size);
	...
	void AddNullTermination(int64_t nPut);
	...
	bool PutOverflow(int64_t nSize);
	bool GetOverflow(int64_t nSize);

	// Does the next bytes of the buffer match a pattern?
	bool PeekStringMatch(int64_t nOffset, const char* pString, int64_t nLen);

	// Peek size of line to come, check memory bound
	int64_t	PeekLineLength();

	// How much whitespace should I skip?
	int64_t PeekWhiteSpace(int64_t nOffset);

	// Checks if a peek get is ok
	bool CheckPeekGet(int64_t nOffset, int64_t nSize);

	// Call this to peek arbitrarily long into memory. It doesn't fail unless
	// it can't read *anything* new
	bool CheckArbitraryPeekGet(int64_t nOffset, int64_t& nIncrement);
...
{
public:
	CUtlInplaceBuffer(int64_t growSize = 0, int64_t initSize = 0, int nFlags = FLAGS_NONE);
...
public:
	...
	//
	bool InplaceGetLinePtr( /* out */ char** ppszInBufferPtr, /* out */ int64_t* pnLineLength);
...
// Where am I reading?
//-----------------------------------------------------------------------------
inline int64_t CUtlBuffer::TellGet() const
{
	return m_Get;
}
...
// How many bytes remain to be read?
//-----------------------------------------------------------------------------
inline int64_t CUtlBuffer::GetBytesRemaining() const
{
...
// What am I reading?
//-----------------------------------------------------------------------------
inline const void* CUtlBuffer::PeekGet(int64_t offset) const
{
...
template <typename T>
inline void CUtlBuffer::GetObjects(T* dest, int64_t count)
{
	for (int64_t i = 0; i < count; ++i, ++dest)
	{
		GetObject<T>(dest);
	}
}
...
inline bool CUtlBuffer::GetTypeText(T& value, int nRadix /*= 10*/)
{
	...
	int64_t nBytesRead = (int64_t)(pEnd - pStart);
	if (nBytesRead == 0)
		return false;
...
inline uint64_t CUtlBuffer::GetUnsignedint64_t()
{
	uint64_t i;
	GetType(i);
	return i;
...
// Where am I writing?
//-----------------------------------------------------------------------------
inline int64_t CUtlBuffer::TellPut() const
{
	return m_Put;
}
...
// What's the most I've ever written?
//-----------------------------------------------------------------------------
inline int64_t CUtlBuffer::TellMaxPut() const
{
	return m_nMaxPut;
}
...
// What am I reading?
//-----------------------------------------------------------------------------
inline void* CUtlBuffer::PeekPut(int64_t offset)
{
...
template <typename T>
inline void CUtlBuffer::PutObjects(T* src, int64_t count)
{
	for (int64_t i = 0; i < count; ++i, ++src)
	{
		PutObject<T>(src);
	}
}
...
inline void CUtlBuffer::PutUnsignedint64_t(uint64_t i)
{
	PutType(i);
}
...
inline int64_t CUtlBuffer::Size() const
{
...
// 
//-----------------------------------------------------------------------------
inline void* CUtlBuffer::AccessForDirectRead(int64_t nBytes)
{
...
inline void	CUtlBuffer::CopyBuffer(const void* pubData, int64_t cubData)
{
	Clear();
	if (cubData)
	{
